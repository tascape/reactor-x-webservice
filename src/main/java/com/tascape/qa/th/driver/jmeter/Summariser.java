 ...
 */
package com.tascape.qa.th.driver.jmeter;

import com.tascape.qa.th.driver.WebServiceJMeter;
 ...
 *
 */
public class Summariser extends org.apache.jmeter.reporters.Summariser
    implements Serializable, SampleListener, TestStateListener, NoThreadClone, Remoteable {
    private static final org.slf4j.Logger LOG = LoggerFactory.getLogger(WebServiceJMeter.class);
    ...
    private SummariserRunningSample result;

    private final SortedSet<Long> responseTimes = new ConcurrentSkipListSet<>();
    ...
    private static class Totals {
        ...
        public final SummariserRunningSample delta = new SummariserRunningSample("DELTA");

        public final SummariserRunningSample total = new SummariserRunningSample("TOTAL");
    ...
    public void sampleOccurred(SampleEvent e) {
        ...
        this.responseTimes.add(s.getEndTime() - s.getStartTime());
    ...
    public void testEnded(String host) {
        ...
        LOG.debug("throughput {} / s", String.format("%.3f", result.getRate()));
        LOG.debug("min        {} ms", result.getMin());
        LOG.debug("average    {} ms", result.getAverage());
        LOG.debug("max        {} ms", result.getMax());
        LOG.debug("error      {} %", result.getErrorPercentage());

        int size = this.responseTimes.size();
        Long[] times = this.responseTimes.toArray(new Long[0]);

        IntStream.of(95, 90, 75, 50, 25, 10, 5).forEachOrdered(i -> {
            LOG.debug("{} % - {} ms", i, times[(int) (size * (i / 100.0))]);
        });
    }

    public SummariserRunningSample getResult() {
        return result;
    }

    public long getResponseTimeByPercentage(int percentage) {
        int size = this.responseTimes.size();
        Long[] times = this.responseTimes.toArray(new Long[0]);
        return times[(int) (size * (percentage / 100.0))];
    }
}
